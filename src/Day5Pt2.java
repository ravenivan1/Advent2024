...
public class Day5Pt2 {
    public static void main(String[] args) {
        ...
        ArrayList<int[]> correctedUpdates = new ArrayList<>();
        int middleSum = 0;
        ...
        for (int[] update : updates) {
            ...
            if (!checkValidUpdate(validRules, update)) correctedUpdates.add(fixUpdate(update, validRules));
            System.out.println(checkValidUpdate(validRules, fixUpdate(update, validRules)));
        }


        for (int[] correctedUpdate: correctedUpdates) {

            middleSum += correctedUpdate[(correctedUpdate.length - 1) / 2];
        }

        System.out.println(middleSum);
    }

    public static int[] fixUpdate(int[] incorrectUpdate, ArrayList<int[]> validRules) {
        boolean fixed = false;
        while (!fixed) {
            for (int[] rule : validRules) {
                ...
                for (int i = 0; i < incorrectUpdate.length; i++) {
                    ...
                    if (incorrectUpdate[i] == rule[1]) {
                        indexSecond = i;
                    }
                }
                if (indexFirst > indexSecond) {
                    int hold = incorrectUpdate[indexFirst];
                    incorrectUpdate[indexFirst] = incorrectUpdate[indexSecond];
                    incorrectUpdate[indexSecond] = hold;
                }
            }
            fixed = checkValidUpdate(validRules, incorrectUpdate);
        }
